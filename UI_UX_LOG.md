...
1. ~~**`paths/[pathId]/module/[modulesId]`** — Violation des règles des Hooks React : `useState`/`useMemo`/`useIsMobile` appelés **après** des `return notFound()` conditionnels.~~ ✅ **CORRIGÉ** : les 4 hooks (+ `useMemo`) sont désormais appelés inconditionnellement en tête de composant, les gardes `notFound()` déplacées après. Build OK.

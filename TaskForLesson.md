...
## [2026-07-16] — DTO record : la validation doit être sur le PARAMÈTRE, pas `[property:]`
**Contexte** : nouveau `record DeclareDomainRequest([property: Required] string Domain)`. Tous les endpoints du controller renvoyaient 500.
**Erreur** : ASP.NET lève `InvalidOperationException: validation metadata defined on property 'Domain' … will be ignored … must be associated with the constructor parameter`. `[property: Required]` place l'attribut sur la propriété générée, mais la validation de modèle valide le PARAMÈTRE du constructeur primaire → métadonnée ignorée + exception, qui casse l'activation de TOUT le controller (500 sur chaque action, pas seulement celle qui bind le DTO).
**Règle** : sur un `record` positionnel utilisé comme corps de requête, poser les DataAnnotations **directement sur le paramètre** (`[Required] string Domain`), jamais `[property: Required]`. Symptôme : 500 sur tous les endpoints d'un controller dès qu'un de ses DTO record a une validation mal ciblée.

## [2026-07-16] — Tests d'intégration : une BDD InMemory au nom FIXE partagée = flakiness sous parallélisme xUnit
**Contexte** : suite à ~127 tests avec 1 échec « flaky » (test Teams) qui variait de run en run ; en ajoutant une 4ᵉ classe basée sur `WebApplicationFactory`, 2-3 échecs intermittents sont apparus sur `AuthMeSuperAdminTests`/`InteractiveVariantsTests`.
**Erreur** : `InteractiveTestApiFactory` faisait `UseInMemoryDatabase("InteractiveVariantsTests")` — nom FIXE. Or `IClassFixture` crée une instance de factory PAR CLASSE, mais les bases EF InMemory sont indexées par **nom au niveau process** → 4 classes de test (`InteractiveVariantsTests`, `AuthMeSuperAdminTests`, `TeamMembersEndpointTests`, `UserTeamsEndpointTests`) partageaient la MÊME base. xUnit exécute les classes en parallèle → contamination croisée des données → échecs non déterministes. Chaque classe passait pourtant SEULE.
**Règle** : chaque `WebApplicationFactory` de test doit utiliser une base InMemory **au nom unique par instance** (`"prefix-" + Guid.NewGuid().ToString("N")`), jamais un nom fixe partagé. Diagnostic d'un échec flaky d'intégration : (1) relancer la classe SEULE (si elle passe → problème d'isolation/parallélisme, pas de logique), (2) `grep UseInMemoryDatabase` pour repérer les noms partagés.

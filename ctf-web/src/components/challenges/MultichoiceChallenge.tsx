        ...
        return (
            <div className="space-y-4">
                ...
                <div style={{
                    ...
                    border: `1px solid ${result.scorePercent >= 70 ? "#10B981" : result.scorePercent >= 40 ? "#f97316" : "#ef4444"}`,
                ...
                {result.results.map(r => {
                    ...
                    const borderColor = isGood ? "#10B981" : isMissed ? "#f97316" : "#ef4444";
                    ...
                    const tagColor    = isGood ? "#10B981" : isMissed ? "#f97316" : "#ef4444";
                    ...
                    return (
                        <div key={r.choiceId} style={{ background: bgColor, border: `1px solid ${borderColor}`, borderRadius: 8, padding: "12px 14px" }}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                                ...
                                <div>
                                    <p style={{ color: "#1E293B", fontSize: 13, fontWeight: 500, margin: 0 }}>{r.label}</p>
                    ...
                    <div style={{ background: "var(--bg-surface)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 8, padding: "14px 16px" }}>
                        ...
                        <ul style={{ padding: 0, margin: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 5 }}>
                            {result.redFlags.map((f, i) => (
                                <li key={i} style={{ fontSize: 13, color: "#334155", display: "flex", gap: 8 }}>
                ...
                {/* Savoir plus */}
                {result.savoirPlus && (
                    <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: 8, padding: "12px 14px" }}>
                        <p style={{ color: "var(--pr)", fontSize: 12, fontWeight: 600, marginBottom: 5 }}>📖 Pour aller plus loin</p>
                        <p style={{ color: "#64748B", fontSize: 13, margin: 0, lineHeight: 1.6 }}>{result.savoirPlus}</p>
                    </div>
                )}
    ...
    return (
        <div className="space-y-5">
                ...
                <div style={{ background: "var(--bg-card)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 10, overflow: "hidden" }}>
                    <div style={{ background: "var(--bg-surface)", borderBottom: "1px solid rgba(59,130,246,0.15)", padding: "14px 18px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                            <div>
                                <p style={{ color: "#1E293B", fontWeight: 600, fontSize: 14, margin: 0 }}>{content.email.from_name}</p>
                                <p style={{ color: "#ef4444", fontSize: 12, fontFamily: "monospace", margin: "3px 0 0" }}>{content.email.from_address}</p>
                            </div>
                            <span style={{ color: "#64748B", fontSize: 11, whiteSpace: "nowrap" }}>{content.email.sent_at}</span>
                        </div>
                        <p style={{ color: "#1E293B", fontWeight: 600, fontSize: 14, margin: "10px 0 2px" }}>{content.email.subject}</p>
                        {content.email.to && <p style={{ color: "#64748B", fontSize: 12, margin: 0 }}>À : {content.email.to}</p>}
                    </div>
            ...
            {content.scenario && !content.email && (
                <div style={{ background: "var(--bg-surface)", borderLeft: "4px solid #f97316", borderRadius: "0 8px 8px 0", padding: "16px 18px", position: "relative" }}>
                    <span style={{ position: "absolute", top: 12, right: 14, fontSize: 20 }}>⚠️</span>
                    <p style={{ color: "#f97316", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 6px" }}>
                        Scénario
                    </p>
                    <p style={{ color: "#1E293B", fontSize: 15, fontWeight: 700, margin: "0 0 10px" }}>{content.scenario.title}</p>
